...
interface LinearState {
  ...
  dimRegion: { activeStart: number; activeEnd: number } | null;
  annotations: Record<string, string>;
  elementGroups: Record<string, string>;
  _pendingRemove?: number;
}

function initState(config: any): LinearState {
  if (!config || !config.data) return { elements: [], pointers: {}, dimRegion: null, annotations: {}, elementGroups: {} };
  const elements: LinearElement[] = config.data.map((val: any, i: number) => {
    ...
    return {
      ...
      opacity: 1, pulse: false, flash: null,
    };
  });
...
function applyStepToState(state: LinearState, step: any, instant: boolean): LinearState {
  ...
  switch (step.type) {
    case 'swap': {
      ...
      const elems = elements.map((el) => ({ ...el, pulse: false, flash: null }));
      const ii = elems.findIndex((e) => elMatch(e, i)); const ij = elems.findIndex((e) => elMatch(e, j));
    ...
    case 'batch-shift': {
      ...
      return { ...state, elements: elements.map((el) => el.index >= start && el.index <= end
        ? { ...el, index: el.index + dir, pulse: false, flash: null } : { ...el, pulse: false, flash: null }) };
    }
    case 'fade-in': {
      const ne = elements.map((el) => ({ ...el, index: el.index >= step.target ? el.index + 1 : el.index, pulse: false, flash: null }));
    ...
    case 'fade-out': {
      const targetEl = elements.find((el) => elMatch(el, step.target));
      const removeIdx = targetEl ? targetEl.index : Number(step.target);
      const ne = elements.map((el) => el.index === removeIdx ? { ...el, opacity: 0, pulse: false, flash: null } : { ...el, pulse: false, flash: null });
    ...
    case 'pointer': {
      ...
      const targetEl = elements.find((el) => elMatch(el, to));
    ...
    case 'compare': {
      ...
      return { ...state, elements: elements.map((el) => elMatch(el, a) || elMatch(el, b)
