...
function renderVisualizations(
  ...
  // Get only the current step's commands for the logger
  const getCurrentStepCommands = (): Command[] => {
    if (currentChunk >= 0 && currentChunk < chunks.length) {
      return chunks[currentChunk].commands;
    }
    return [];
  };
  ...
  // Layout: side-by-side on desktop (visual tracers left, logs right)
  return (
    <div className="flex flex-col md:flex-row gap-4 h-full">
      {/* Left side - Visual components (50% width) */}
      ...
      {/* Right side - Cumulative logger (50% width) */}
      {hasLog && (
        <div className="flex-1 min-w-0 border-t md:border-t-0 md:border-l border-gray-700 pt-4 md:pt-0 md:pl-4">
          <LogRenderer commands={commands} currentStepCommands={getCurrentStepCommands()} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className={`flex flex-col gap-4 ${className}`}>
      ...
      <div className="flex-1">
        {renderVisualizations(getCurrentCommands(), tracerTypes, chunks, currentChunk)}
      </div>
