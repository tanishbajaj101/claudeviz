...
const backendOutputPath = path.join(process.cwd(), 'packages/backend/src/data/problems/all_problems.ts');
const frontendOutputPath = path.join(process.cwd(), 'packages/frontend/src/data/problems/all_problems.ts');

const problemsJson = JSON.parse(fs.readFileSync(problemsJsonPath, 'utf8'));

const problemMetadataMap = {
...
function inferTitle(problem) {
    if (problem.title && problem.title.trim().length > 0) return problem.title;
    ...
    if (desc.includes("combination sum") && !desc.includes("ii")) return "Combination Sum";
    if (desc.includes("combination-sum-ii") || (desc.includes("combination sum") && desc.includes("ii"))) return "Combination Sum Ii";
    if (desc.includes("course schedule") && !desc.includes("ii")) return "Course Schedule";
    ...
    if (desc.includes("word search") && !desc.includes("ii")) return "Word Search";
    ...
    if (desc.includes("longest repeating character replacement")) return "Longest Repeating Character Replacement";
    if (desc.includes("permutation in string")) return "Permutation In String";
    if (desc.includes("min stack")) return "Min Stack";
    if (desc.includes("generate parentheses")) return "Generate Parentheses";
    if (desc.includes("koko eating bananas")) return "Koko Eating Bananas";
    if (desc.includes("search in rotated sorted array")) return "Search In Rotated Sorted Array";
    if (desc.includes("find minimum in rotated sorted array")) return "Find Minimum In Rotated Sorted Array";
    if (desc.includes("merge two sorted lists")) return "Merge Two Sorted Lists";
    if (desc.includes("add two numbers")) return "Add Two Numbers";
    if (desc.includes("maximum depth of binary tree")) return "Maximum Depth Of Binary Tree";
    if (desc.includes("diameter of binary tree")) return "Diameter Of Binary Tree";
    if (desc.includes("balanced binary tree")) return "Balanced Binary Tree";
    if (desc.includes("binary tree level order traversal")) return "Binary Tree Level Order Traversal";
    if (desc.includes("validate binary search tree")) return "Validate Binary Search Tree";
    if (desc.includes("kth smallest element in a bst")) return "Kth Smallest Element In A BST";
    if (desc.includes("last stone weight")) return "Last Stone Weight";
    if (desc.includes("kth largest element in an array")) return "Kth Largest Element In An Array";
    if (desc.includes("design twitter")) return "Design Twitter";
    if (desc.includes("subsets") && !desc.includes("ii")) return "Subsets";
    if (desc.includes("subsets ii")) return "Subsets Ii";
    if (desc.includes("palindrome partitioning")) return "Palindrome Partitioning";
    if (desc.includes("graph valid tree")) return "Graph Valid Tree";
    if (desc.includes("word ladder")) return "Word Ladder";
    if (desc.includes("alien dictionary")) return "Alien Dictionary";
    if (desc.includes("reconstruct itinerary")) return "Reconstruct Itinerary";
    if (desc.includes("min cost to connect all points")) return "Min Cost To Connect All Points";
    if (desc.includes("house robber ii")) return "House Robber Ii";
    if (desc.includes("palindromic substrings")) return "Palindromic Substrings";
    if (desc.includes("coin change") && !desc.includes("ii")) return "Coin Change";
    if (desc.includes("maximum product subarray")) return "Maximum Product Subarray";
    if (desc.includes("coin change ii")) return "Coin Change Ii";
    if (desc.includes("target sum")) return "Target Sum";
    if (desc.includes("longest increasing path in a matrix")) return "Longest Increasing Path In A Matrix";
    if (desc.includes("burst balloons")) return "Burst Balloons";
    if (desc.includes("jump game ii")) return "Jump Game Ii";
    if (desc.includes("gas station")) return "Gas Station";
    if (desc.includes("merge triplets to form target triplet")) return "Merge Triplets To Form Target Triplet";
    if (desc.includes("partition labels")) return "Partition Labels";
...
const processedProblems = problemsJson.map((p) => {
    ...
    return {
        ...
        testCases: p.examples.map((ex) => ({
            ...
            expectedOutput: ex.output.replace(/\[|\]/g, "").replace(/,/g, " ").trim()
        })),
        judge0Limits: {
            cpu_time_limit: 2,
            wall_time_limit: 5,
...
}).filter(p => p.title !== "Unknown Problem");

// For backend (CommonJS/ESM mixed)
const backendHeader = 'import { Problem } from "@algoarena/shared";\n\nexport const importedProblems: Problem[] = '
const footer = ';\n';
const json = JSON.stringify(processedProblems, null, 4);
fs.writeFileSync(backendOutputPath, backendHeader + json + footer);

// For frontend (ESM)
const frontendHeader = 'import { Problem } from "@/types";\n\nexport const importedProblems: Problem[] = '
fs.writeFileSync(frontendOutputPath, frontendHeader + json + footer);

console.log('Successfully processed ' + processedProblems.length + ' problems and saved to backend and frontend.');
